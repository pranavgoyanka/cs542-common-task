...
for offset in range(15):
    for city in cities:
        ...
        pred_date = pd.to_datetime(end_date, format="%Y-%m-%d") + timedelta(days=offset)
        #   pred_date = end_date
        pred = getPrediction(city, offset=-offset)
        prediction_results.append(
            {"date": str(pred_date)[:10], "city": city, "tmax_predicted": pred}
        )
df_predictions = pd.DataFrame(prediction_results)
df_predictions.to_csv("predictions_final_all_days.csv", mode="a", header=False)
